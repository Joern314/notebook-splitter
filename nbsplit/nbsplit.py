...
def parse(inputfile, keep, remove, basekey, linestarts):
    ...
    Also remove cell/line linestarts.
    """
    ...
    for cell in notebook["cells"]:
        ...
        if keepcell == True:
            ...
            if type(cell_new['source']) is str:
                cell_new['source'] = remove_linestarts(cell_new['source'], linestarts)
            else: # list of string
                cell_new['source'] = [remove_linestarts(part, linestarts) for part in cell_new['source']]
            
            notebook_new["cells"].append(cell_new)
        else:
...
def main():
    ...
    parser.add_argument('--line',   "-l", action="append", help="Remove all lines that start with these strings. (default: None)")
    ...
    notebook = parse(inputfile=args.infile, keep=args.keep, remove=args.remove, basekey=args.basekey, linestarts=args.line)
